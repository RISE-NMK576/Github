...
namespace Netflixx.Controllers
{
    public class LoginController : Controller
    {
        ...
        public async Task<IActionResult> SendSignUpOtp([FromBody] EmailRequest request)
        {
            try
            {
                _logger.LogInformation($"Dữ liệu nhận được: {JsonSerializer.Serialize(request)}");

                // Kiểm tra request null
                if (request == null)
                {
                    _logger.LogWarning("Request body là null");
                ...
                {
                    var errors = ModelState.Values.SelectMany(v => v.Errors).Select(e => e.ErrorMessage);
                    _logger.LogWarning($"ModelState không hợp lệ: {string.Join(", ", errors)}");
