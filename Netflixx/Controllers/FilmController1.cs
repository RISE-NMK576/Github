    ...
    public class FilmController1 : Controller
    {
        ...
        public async Task<IActionResult> Create(FilmsModel film, IFormFile posterFile)
        {
            try
            {
                ...
                // Kiểm tra ít nhất một trong hai phương thức poster được cung cấp
                if (string.IsNullOrEmpty(film.PosterUrl) && posterFile == null)
                {
                    ModelState.AddModelError("", "Vui lòng cung cấp URL poster hoặc tải lên file poster");
                }
                ...
                if (posterFile != null && posterFile.Length > 0)
                {
                    ...
                    {
                        ModelState.AddModelError("PosterUrl", "Kích thước file không được vượt quá 5MB");
                        SetupCreateEditViewBags();
                        return View("~/Views/Film/ManagerFilm/CreateFilms.cshtml", film);
                    }
                    ...
                    {
                        ModelState.AddModelError("PosterUrl", "Chỉ chấp nhận file ảnh (JPG, JPEG, PNG, GIF)");
                        SetupCreateEditViewBags();
                        return View("~/Views/Film/ManagerFilm/CreateFilms.cshtml", film);
                    }
                    ...
                    film.PosterUrl = "/uploads/posters/" + uniqueFileName;
                }
                ...
                // Thay vì chuyển hướng, trả về view Create với model mới
                SetupCreateEditViewBags();
                return View("~/Views/Film/ManagerFilm/CreateFilms.cshtml", new FilmsModel());
            }
            catch (Exception ex)
        ...
        {
            // Bỏ qua validation cho các trường tracking
            ...
            try
            {
                ...
                // Lưu giá trị Rating hiện tại trước khi cập nhật
                ...
                film.ModifiedFields = GetModifiedFields(_context.Entry(film).OriginalValues, _context.Entry(film).CurrentValues);
                ...
                return Json(new
                {
                    ...
                    film = new
                    {
                        ...
                        rating = film.Rating // Trả về rating để cập nhật UI nếu cần
                    }
                });
            }
            catch (Exception ex)
        ...
        public async Task<IActionResult> DeletePermanent(int id)
        {
            try
            {
                ...
                {
                    var filePath = Path.Combine(_hostEnvironment.WebRootPath, film.PosterUrl.TrimStart('/'));
                    if (System.IO.File.Exists(filePath))
                    {
                        System.IO.File.Delete(filePath);
                    }
